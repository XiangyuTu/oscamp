...
fn sys_mmap(
    ...
    let size: usize = length.align_up_4k();

    let va = if addr.is_null() {
        match aspace.find_free_area(
            aspace.base() + PAGE_SIZE_4K, // avoid NULL
            size,
            memory_addr::AddrRange::new(aspace.base(), aspace.end()),
        ) {
            Some(addr) => addr,
            None => {
                ax_println!("sys_mmap: ENOMEM");
    ...
    ax_println!("sys_mmap: va={:#x?}, size={:#x?}", va, size);

    let mut flags = MappingFlags::from(MmapProt::from_bits_truncate(flags));
    flags.set(MappingFlags::READ, true);
    ax_println!("sys_mmap: flags={:?}", flags);
    
    if let Err(err) = aspace.map_alloc(va, size, flags, true) {
        ax_println!("sys_mmap: {:?}", err);
        return -err.code() as _;
    }

    if fd > 0 {
        api::sys_read(fd, va.as_usize() as *mut _, length);
    }

    ax_println!("returning va={:#x?}", va);
