...
//! different sizes and a [rlsf] for blocks larger than 4096
//! bytes.
//!
//! It's based on <https://github.com/weclaw1/slab_allocator>.
//!
//! [rlsf]: https://docs.rs/rlsf/latest/rlsf/

extern crate alloc;

use alloc::alloc::{AllocError, Layout};
use core::ptr::NonNull;
use allocator::{BaseAllocator, ByteAllocator, TlsfByteAllocator};
...
impl Heap {
    ...
    pub unsafe fn add_memory(&mut self, heap_start_addr: usize, heap_size: usize) {
        ...
        self.tlsf_allocator
            .add_memory(heap_start_addr, heap_size);
    }
    ...
    unsafe fn _grow(&mut self, mem_start_addr: usize, mem_size: usize, slab: HeapAllocator) {
        match slab {
            ...
            HeapAllocator::TlsfByteAllocator => self
    ...
    pub fn usable_size(&self, layout: Layout) -> (usize, usize) {
        match Heap::layout_to_allocator(&layout) {
            ...
            HeapAllocator::TlsfByteAllocator => (layout.size(), layout.size()),
        }
    }
